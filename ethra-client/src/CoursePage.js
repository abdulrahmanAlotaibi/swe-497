...
class DashboardCourse extends Component {
  ...
  async getMembership() {
    ...
    try {
      const res = await axios.get(`https://ethra.herokuapp.com/student/${id}`, {
  ...
  async getInfo() {
    ...
    try {
      ...
      course.img = `https://ethra.herokuapp.com/${course.img}`;
      for (let i = 0; i < videos.length; i++) {
        videos[i].content = `https://ethra.herokuapp.com/${videos[i].content}`;
      }
  ...
  addToCart = async () => {
    try {
      ...
      await axios.post(
        `https://ethra.herokuapp.com/course/${course._id}/add-to-cart`,
        null,
        { headers: { authorization: `Bearer ${token}` } }
      );
  ...
  handlePaidVideo = async i => {
    ...
    const { role, id } = JSON.parse(window.localStorage.getItem("account"));
    if (role === "admin") {
        ...
        <div className="course-page-info-container">
          <div className="course-page-info">
            ...
            <div className="course-page-info__rating-container">
              ...
              <div className="reviews__rating">{getRatingJSX(rating)}</div>
            </div>
            <div className="course-page-info__meta">
              ...
              <span className="course-page-info__desc">Description: {description}</span>
              <div>
                <span>
                  Start Date:{" "}
