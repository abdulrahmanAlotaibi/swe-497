...
export default class StreamDashboard extends React.Component {
  ...
  getVideos = async () => {
    try {
      ...
      const res = await axios.get(`https://ethra.herokuapp.com/course/${id}/videos`);
      const videos = res.data.data;
      // for (let i = 0; i < videos.length; i++) {
      //   videos[i] = `https://ethra.herokuapp.com/${videos[i]}`;
      // }
  ...
  deleteVideo = async (id, index) => {
    try {
      await axios.delete(`https://ethra.herokuapp.com/course/videos/${id}`);
  ...
  uploadVideo = async () => {
    try {
      ...
      const res = await axios.post(
        `https://ethra.herokuapp.com/course/${id}/upload-video`,
        fd
      );
      const newVideos = [...this.state.videos];
  ...
  handleChangeSubmission = async () => {
    try {
      ...
      const res = await axios.patch(
        "https://ethra.herokuapp.com/course/videos/update",
        { videos }
      );
