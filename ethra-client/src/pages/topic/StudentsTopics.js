...
import StudentTopic from "pages/topic/StudentTopic";
export default class extends Component {
  ...
  async handleSubmitReview() {
    ...
    try {
      const res = await axios.post(
        `http://localhost:8000/course/${selectedCourse._id}/add-review`,
        {
          text: textReview,
          rating: ratingNumber
        },
  ...
  render() {
    ...
    if(courses.filter(course => course.type === "topic").length === 0) {
        return <h2>There are no topics</h2>
    }
    const renderedCourses = this.state.courses
      ...
      .map(course => {
        return (
          <>
            <StudentTopic course={course} owned={true} />
          </>
        );
      });
    ...
    return (
      <section className="student-courses-section">
        <h2 className="student-courses-section__primary-heading">My Topics</h2>
